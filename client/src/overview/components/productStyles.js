...
var productStyles = ({currentProduct, styles, currentStyle, changeStyles, changeStyle}) => {
  ...
  return (
    <div id = 'productInfo'>
      ...
      <label>Size:
        <span id = 'size'>
          <select onChange = {(e) => currentSku[1](currentStyle.skus[e.target.value])}>
            <option>Select Size</option>
            {(currentStyle.skus) ? Object.keys(currentStyle.skus).map((keyName) => (
      ...
      <label>Qty:
        <span id = 'quantity'>
          <select>
            {(currentSku[0]) ? [...Array(currentSku[0].quantity)].map((e, i) => {
